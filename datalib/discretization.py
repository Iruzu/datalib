...
def discretizeEF(x, num_bins):  
    ...
    if len(cut_pt)==1 and cut_pt[1]==max(x):   
...
def discretize (data,num_bins, disc_alg ="EW"):
    ...
    Returns
    -------
    data (pd.DataFrame): Dataframe with discretized values and a column anmed "Cut_p" with the cut points.
    """
    data = data2df (data)
    if is_num(data):
