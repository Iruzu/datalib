...
def entropy(x, normalize=False):
    ...
    prob = np.delete(prob, np.argwhere(prob == 0)) # honegaz ya no hace falta meterle meterle nansum y no salta warning. mirar si merece ponerle condicion
...
def calc_metrics (data):
    ...
    Returns
    -------
    data_new (pd.DataFrame): The input data with aditional columns that contain the calculated metric according to the tipe of data.
    """
    ...
    elif is_cont(data_new): # en si puede ser con cualquier cosa mientras sea numerico no?                   
        data_new['variance'] = data_new.apply(variance, axis=1)
        return(data_new)
    
    elif not is_cont(data_new):  # integer bebai hamen sartu ze pide lo de continua pero...   
        data_new['entropy'] = data_new.apply(entropy, axis=1)
        return(data_new)
        
    else:
        print("The input is not appropiate. Please check it")
